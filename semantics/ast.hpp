...
class AST {
public:
  void type_check(Type t){ //sthn minibasic mporoume einai aplh glwssa
    sem();
...
public:
  ...
  virtual void sem() override {
    left->type_check(TYPE_bool);
    right->type_check(TYPE_bool);
    type=TYPE_bool;
  }
private:
  Cond *left;
...
public:
  ...
  virtual void sem() override{
    SymbolEntry *er = st.lookup(*var);
    type = er->type;
    expr->type_check(er->type);
  }
private:
  char* var;
  Expr *expr;
...
public:
  Finish_2(Expr *e=nullptr): expr(e){}
  virtual void printOn(std::ostream &out) const override {
    out<<" return"; 
    if(expr!=nullptr)out<<*expr;   
    out<<"; "; 
